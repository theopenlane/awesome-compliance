...
- [dora-lei-check](https://github.com/kakoullig-hue/dora-lei-check) - Pre-submission check for the LEIs in a DORA Register of Information. Resolves every LEI against the live GLEIF register and names the EBA rule code each failure will trigger (VR_2, VR_12, VR_23, VR_71, VR_77), so a lapsed provider LEI is caught before submission rather than in a rejection. Runs locally — the register never leaves your machine, only the public LEI codes are looked up; fully offline via GLEIF's daily golden-copy file. Python, zero dependencies, MIT. ([Docs](https://kakoullig-hue.github.io/dora-lei-check/))
